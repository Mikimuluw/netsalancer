...
export async function generatePDF(data: PDFFormData, etbRate?: number): Promise<void> {
  ...
  // =============================================
  doc.addPage();
  y = addHeader(2, "FXD/04/2026 — Key Provisions Summary");

  const provisions = [
    {
      title: "Article 6.2 — 100% FX Retention",
      text: "Any service exporter is entitled to hold 100 percent of its export proceeds in an FX retention account for an indefinite period. This means you can keep all your USD earnings in a bank account permanently — no forced conversion.",
    },
    {
    ...
    },
    {
      ...
      text: "Banks are authorized to issue international Visa or Mastercard cards linked to FX retention accounts. This allows direct international spending — for tools, subscriptions, shopping, and services — without conversion.",
    },
    {
  ...
  provisions.forEach((p) => {
    if (y > pageH - 50) {
      doc.addPage();
      y = addHeader(2, "FXD/04/2026 — Key Provisions (continued)");
      addFooter(2, 5);
    }
  ...
  const checklistItems = [
    ...
    },
    {
      ...
      items: [
        "TIN Certificate (if registered)" + (data.tinNumber ? ` — Your TIN: ${data.tinNumber}` : ""),
        "Business / Trade License (if applicable)",
  ...
  steps.forEach((step, i) => {
    ...
    const detailLines = step.details.map((d) =>
      doc.splitTextToSize("• " + d, contentW - 20)
    );
  ...
  doc.setFontSize(10);
  doc.setTextColor(0, 100, 50);
  doc.text(`${data.preferredBank} — FX Department`, margin + 5, y + 8);

  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  doc.setTextColor(60, 60, 60);
  doc.text(`📞 ${bank.phone}`, margin + 5, y + 15);
  doc.text(`📧 ${bank.email}`, margin + 5, y + 21);
  doc.text(`📍 ${bank.branch}`, margin + 5, y + 27);
  ...
  doc.setTextColor(60, 60, 60);
  doc.setFontSize(9);
  doc.text("📧 mikimuluw@gmail.com  |  🌐 netsalancer.et", margin + 5, y + 15);
